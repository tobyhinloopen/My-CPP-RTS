...
TEST_CASE("unit_health_manager") {
  ...
  REQUIRE(thing_a->health() == 10.0);
  REQUIRE(thing_b->health() == 10.0);
  REQUIRE(unit_health_manager.health() == 20.0);
  REQUIRE(unit_health_manager.alive());
  REQUIRE(!unit_health_manager.dead());
  REQUIRE(unit_health_manager.initial_health() == 20.0);

  SECTION("With all components set to 0 health") {
    thing_a->apply_damage(10.0);
    thing_b->apply_damage(10.0);
    REQUIRE(thing_a->health() == 0.0);
    REQUIRE(thing_b->health() == 0.0);
    REQUIRE(unit_health_manager.health() == 0.0);
    REQUIRE(!unit_health_manager.alive());
    REQUIRE(unit_health_manager.dead());
    REQUIRE(unit_health_manager.initial_health() == 20.0);
  }
